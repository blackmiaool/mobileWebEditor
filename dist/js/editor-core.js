...
angular.module("main", ["dndLists", "contenteditable", "ngAnimate", "ngRoute", "angularFileUpload"]).animation('.editor-document-slide', function () {
    ...
    $http({
        ...
        //                }
        //            )
        if (!data.result) alert("先打开support，再打开这个。你没登录。");
    ...
    uploader.onCompleteAll = function () {
        ...
        if (upload_mode == "you_pure_upload_pic") {} else {
    ...
    $rootScope.$on("appendModel", function (e, model) {
        ...
        $scope.models.lists.forEach(function (v, i) {
            if (v.editor_selected) last_index = i + 1;
        });
    ...
    $rootScope.$on("syncModel", function (e, model) {
        //            console.log(model);
        //            console.log(typeof(model))
        $scope.models.lists.forEach(function (m, index) {
    ...
    instance.editor_tools = [{
        ...
        check: function (index) {
            if (index < list.length - 1) return true;else {
                return false;
            }
        }
    }, {
    ...
    angular.forEach(this.editor_tools, function (tool, index) {
        tool.callback_wrap = function (index) {
            if ((!this.check || this.check(index)) && this.callback) return this.callback.apply(this, arguments);
        };
    ...
    function update_preview() {
        ...
        //            console.log("mmwwmm")
        if (preview_timeout != "none") clearTimeout(preview_timeout);
        preview_timeout = setTimeout(function () {
            ...
            if (sysSettingData.value.clean_mode) {
                ...
                if (contariner.length) preview_load(contariner[0].outerHTML);
            }
    ...
    function blockIndent(editor, from, to) {
        editor.operation(function () {
            for (var i = from; i < to; ++i) editor.indentLine(i, "smart");
        });
    }
    var formvalue = `{
    "schema": {
 "data": {
    ...
    $scope.implement = function () {
        ...
        if (!form_err) {
            $("#editor-model-form-test .jsonform-submit").click(function () {
                console.log("www");
                test_scope.$apply(function (scope) {
...
$(function () {
    ...
    function ass_panel_product_create(dom) {
        ...
        container.find(".search input").keyup(function () {
            var keys = ["gid", "name"];
        ...
        function set_table(data) {
            function table_create(data) {
                ...
                for (var i in data) {
                    ...
                    var content = ["gid", false, "name", false, "gid", function (data) {
        ...
        panel.enter = function () {
            ...
            $.get("http://support.io.mi.srv/shop/pipe", {
                k: "test",
                m: "Shopv2",
                a: "getHotList"
            }, function (data) {
        ...
        function set_table(data) {
            function table_create(data) {
                ...
                for (var i in data) {
                    ...
                    var content = ["id", false, "title", false, "url", false];
    ...
    $("body").keydown(function (e) {
        if (e.keyCode == 112) {
            if ($(":focus").length) showAssMenu($(":focus"));
            return false;
        }
    });
});